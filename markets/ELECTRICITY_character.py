"""Market character: ELECTRICITY. Research layer only; no hard-coded market result."""
...
MARKET_CHARACTER = MarketCharacter(
    market='ELECTRICITY',
    market_type='power_market',
    identity='Electricity market connecting generation, fuel mix, grid availability, industrial demand, weather, tariffs and power prices.',
    core_drivers=('load', 'generation availability', 'fuel cost', 'renewables', 'hydrology', 'weather', 'transmission', 'outages', 'industry'),
    upstream=('coal', 'natural gas', 'hydro', 'solar', 'wind', 'nuclear where applicable', 'generation assets'),
    processing=('generation', 'dispatch', 'transmission', 'distribution', 'storage'),
    downstream=('industry', 'commercial', 'residential', 'railways', 'data centres', 'telecom', 'utilities'),
    demand_sectors=('manufacturing', 'metals', 'cement', 'chemicals', 'IT/data centres', 'telecom', 'railways', 'commercial'),
    supply_chain_channels=('fuel -> generation', 'generation -> grid', 'grid -> industry', 'power price -> production cost'),
    direct_exposure_patterns=('power generator', 'power distributor', 'large industrial consumer', 'data-centre operator', 'renewable developer'),
    indirect_exposure_patterns=('production disruption', 'energy cost inflation', 'capacity expansion', 'grid capex', 'renewable transition'),
    cost_channels=('electricity tariff', 'open-access power', 'fuel', 'transmission', 'backup generation'),
    revenue_channels=('generation volume', 'merchant price', 'regulated tariff', 'grid capex', 'renewable capacity'),
    macro_channels=('industrial production', 'weather', 'inflation', 'rates', 'policy', 'fuel prices'),
    global_channels=('global gas', 'coal', 'renewables equipment', 'weather', 'grid investment', 'data-centre demand'),
    event_triggers=('plant outage', 'tariff revision', 'capacity commissioning', 'grid order', 'renewable PPA', 'fuel shock'),
    indicators=('power demand', 'generation mix', 'market clearing price', 'PLF', 'renewable generation', 'coal/gas price', 'peak demand', 'outages'),
