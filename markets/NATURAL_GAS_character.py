"""Market character: NATURAL_GAS. Research layer only; no hard-coded market result."""
...
MARKET_CHARACTER = MarketCharacter(
    market='NATURAL_GAS',
    market_type='energy_feedstock',
    identity='Natural-gas market connecting power, fertilizer, city gas, LNG, industrial heat and global gas pricing.',
    core_drivers=('weather', 'power demand', 'LNG flows', 'pipeline supply', 'storage', 'industry', 'fertilizer', 'geopolitics'),
    upstream=('gas fields', 'associated gas', 'pipelines', 'LNG terminals'),
    processing=('gas processing', 'LNG liquefaction', 'regasification', 'compression'),
    downstream=('power', 'fertilizer', 'CNG/PNG', 'city gas', 'industrial heat', 'chemicals'),
    demand_sectors=('utilities', 'fertilizer', 'chemicals', 'city gas', 'manufacturing'),
    supply_chain_channels=('wellhead -> pipeline', 'LNG -> regas', 'gas -> power', 'gas -> fertilizer'),
    direct_exposure_patterns=('gas producer', 'gas distributor', 'LNG importer', 'gas-fired power', 'fertilizer feedstock'),
    indirect_exposure_patterns=('electricity tariff', 'fertilizer cost', 'industrial margin', 'inflation', 'power availability'),
    cost_channels=('gas procurement', 'LNG import', 'power', 'transport'),
    revenue_channels=('gas sales', 'LNG throughput', 'power generation', 'fertilizer output'),
    macro_channels=('inflation', 'industrial production', 'electricity demand', 'INR/USD', 'subsidies'),
    global_channels=('Henry Hub', 'JKM/LNG', 'Europe gas', 'LNG shipping', 'weather', 'China/Europe demand'),
    event_triggers=('LNG contract', 'gas allocation', 'power utilization', 'fertilizer capacity', 'pipeline disruption'),
    indicators=('gas return', 'storage', 'weather degree-days', 'LNG imports', 'power demand', 'industrial PMI', 'FX'),
